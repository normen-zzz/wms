								...
								<div class="card">
									<div class="card-header">
										...
										<form id="finishProduction">
											...
											<button type="submit" class="btn btn-success">Finish Picking</button>
									...
									<div class="card-body">
										<h1>Production: <?= $production['no_production'] ?></h1>
												...
												<tbody>
													<?php foreach ($items as $item) : ?>
														...
														<tr>
															<td><?= $item['sku'] ?></td>
															<td><?= $item['nama_barang'] ?>
																<input type="text" hidden name="sku" value="<?= $item['sku'] ?>">
																<input type="text" hidden name="id_material" hidden value="<?= $item['id_material'] ?>">
															</td>


															<td><?= $item['batchnumber'] ?><input type="number" hidden name="id_batch" value="<?= $item['batch_id'] ?>"></td>
  ...
  `;
		...
		$('.submitRow').on('click', function() {
			...
			$(this).closest('td').prev('td').find('table.addRackRow').find('tbody tr').each(function() {
				...
				if (qty == '') {
					...
					console.log(id_material, sku, id_batch, rack, qty);
					data.push({
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url('user/Production/pickFromRackProcess/' . $production['id_production']) ?>',
				data: {
					data: data
				},
				dataType: 'json',
	...
	<script>
		$('.get-recommendations').on('click', function(e) {
			...
			$.ajax({
				url: '<?= base_url("user/Production/getAvailableRack") ?>',
				method: 'POST',
				data: {
					sku: sku,
					id_batch: id_batch,
		...
		function checkQtyOnRack() {
			$('table.addRackRow tbody').off('keyup', 'input[name="qty[]"]').on('keyup', 'input[name="qty[]"]', function() {
				...
				setTimeout(function() {
					...
					$.ajax({
						url: '<?= base_url("user/Production/getQuantityRackItems") ?>',
						method: 'POST',
						data: {
							sku: sku,
							id_batch: id_batch,
	...
	<!-- finishProduction -->
	<script>
		$('#finishProduction').on('submit', function(e) {
			e.preventDefault();
			Swal.fire({
				title: 'Confirm',
				text: 'Are you sure you want to finish this production?',
				icon: 'warning',
				showCancelButton: true,
				confirmButtonText: 'Yes',
				cancelButtonText: 'No',
				...
				if (result.isConfirmed) {
					// ajax 
					$.ajax({
						url: '<?= base_url('user/Production/finishProduction/' . $production['id_production']) ?>',
						method: 'POST',
						dataType: 'json',
						success: function(response) {
							Swal.fire({
