...
<body>
									...
									<div class="card-body">
										<form id="production-form">
											...
											<div class="form-group mb-3">
												...
												<input type="text" class="form-control" id="batch_bundling" name="batch_bundling" placeholder="Batch Bundling">
												<!-- quantity  -->
												<label for="quantity_bundling">Quantity</label>
												<input type="number" class="form-control" id="quantity_bundling" name="quantity_bundling" placeholder="Quantity">

												<button type="button" id="search-bundling" class="btn btn-primary">Search</button>
											</div>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                data.forEach(function(item) {
                    // Ambil data batch berdasarkan sku_material
                    $.ajax({
                        url: '<?= base_url() ?>user/production/getBatchMaterial', // Endpoint untuk mengambil batch
                        type: 'post',
                        data: {
                        ...
                        success: function(batchResponse) {
                            ...
                            // Buat option untuk select batch
                            batches.forEach(function(batch) {
                                batchOptions += `<option value="${batch.id_batch}">${batch.batchnumber}</option>`; // Sesuaikan dengan struktur data batch
                            });

                            // Append row ke material-rows
	...
	<script>
		function addRowBatchMaterial() {
			$('.addRowBatchMaterial').click(function() {
