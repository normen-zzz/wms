                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form id="purchaseorderEditForm">
                                            <select name="customer" id="customer" class="form-select selectCustomer">
                                                <?php foreach ($customer->result_array() as $customer1) { ?>
                                                    <option <?php if ($customerPo['id_customer'] == $customer1['id_customer']) {
                                                                echo 'selected';
                                                            } ?> value="<?= $customer1['id_customer'] ?>"><?= $customer1['nama_customer'] ?></option>
                                                <?php } ?>
                                            </select>
                                            <div class="table-responsive">
                                                <table class="table table-dark table-striped mt-2" id="table">
                                                    <thead>
                                                        <tr>
                                                            <th>SKU</th>
                                                    ...
                                                    <tbody id="table-body">
                                                        <?php foreach ($detailPo->result_array() as $detailPo1) { ?>
                                                            <tr>
                                                                <td><?= $detailPo1['sku'] ?> | <?= $detailPo1['nama_barang'] ?></td>

                                                                <td><?= $detailPo1['batchnumber'] ?></td>
                                                                <td><?= date('d-m-Y', strtotime($detailPo1['expiration_date']))  ?></td>
                                                                <td><?= $detailPo1['qty'] ?></td>
                                                                <td><button type="button" style="border: none; background:none" data-id_datapurchaseorder="<?= $detailPo1['id_datapurchaseorder'] ?>" class="deleteDataPurchaseorder"><i class="badge-circle font-medium-1" style="color: red;" data-feather="trash"></i></button></td>
                                                            </tr>
                                                        <?php } ?>
                                                    </tbody>
                                                    <tfoot>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#purchaseorderEditForm').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: "<?= base_url('user/purchaseorder/editPurchaseorder/' . $uuid) ?>",
                    type: "POST",
                    data: $(this).serialize(),
                    dataType: 'json',
                    success: function(response) {
                        Swal.fire({
                            ...
                            if (response.status === 'success') {
                                $('#purchaseorderEditForm')[0].reset();
                                window.location.href = "<?= base_url('user/purchaseorder/edit/' . $uuid) ?>";
                            }
                        });
                    },
            ...
            $('.deleteDataPurchaseorder').on('click', function(e) {
                if (confirm('Apakah anda yakin mau menghapus..?')) {
                    e.preventDefault();
                    var id_datapurchaseorder = $(this).data("id_datapurchaseorder")
                    $.ajax({
                        url: "<?= base_url('user/purchaseorder/deleteDataPurchaseorder') ?>",
                        type: "POST",
                        dataType: "JSON",
                        data: {
