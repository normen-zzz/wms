        ...
        $(document).ready(function() {
            ...
            $('#stocktransferForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    url: "<?= base_url('user/stocktransfer/insertStocktransfer') ?>",
                    type: "POST",
                    data: $(this).serialize(),
                    dataType: 'json',
            ...
            function initSelect2AndFlatpickr() {
                $('.selectBarang').select2({
                    ajax: {
                        url: '<?= base_url('user/stocktransfer/getDataBarangSelect') ?>',
                        type: "POST",
                        dataType: 'json',
                        delay: 250,
                ...
                $('.selectBarang').on('change', function() {
                    ...
                    $.ajax({
                        url: '<?= base_url('user/stocktransfer/getBatch') ?>',
                        type: 'POST',
                        data: {
                            barangId: barangId
                        },
                ...
                $('.qty').on('keyup', function() {
                    ...
                    //rack
                    var rack = row.find('.from'); // Get the selectBatch element in the current row
                    var lastqty = row.find('.qty');

                    $.ajax({
                        url: '<?= base_url('user/stocktransfer/checkQty') ?>',
                        type: 'POST',
                        data: {
                ...
                $('.recommendedRack').on('click', function() {
                    ...
                    $.ajax({
                        url: '<?= base_url('user/stocktransfer/get_rack_recommendations') ?>',
                        type: 'POST',
                        data: {
                            id_barang: id_barang,
                            id_batch: id_batch,
